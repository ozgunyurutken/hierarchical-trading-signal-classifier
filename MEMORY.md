...
## V5 Phase G/H/I/J — Improvement Triple (2026-05-10) — **NOT ADOPTED**

> 🚫 **Karar (2026-05-11):** Bu fazlar denendi ama paper'a dahil edilmiyor. Phase G küçük BTC lift (Sharpe +0.051) verdi ama ETH'de etki yok, Phase H+I'de F1 düzeldi ama Sharpe düştü (paradoks). 4-6 sayfa IEEE'de odak dağıtıcı. **Paper Phase 5.1 baseline ile finalize ediliyor.**
>
> Aşağıdaki sonuçlar git history'de dokümantasyon olarak kalır, ileride faydalı olabilir.
...
**Pre-improvement checkpoint:** tag `v5-pre-improvements-2026-05-10` (commit `33b59f7`) — paper bu state'i kullanır.
...
### Paper-ready findings — Phase 5.1 BASELINE ONLY (Phase G/H/I/J bulguları paper'a girmiyor)

Paper'da kullanılacak:
1. **Asset-specific architecture optimum** (Phase 5.1): BTC = 3stage_full, ETH = flat. No-free-lunch, paper'ın Discussion ana noktası.
2. **RSI/MACD model ≠ classical TA** (Phase 6 W&L gözlem): RSI>70 → +3.25% mean fwd return on BTC. Verified data-driven divergence, paper'a Discussion finding.

Paper'a girmeyen (Phase G/H/I/J — git history'de dokümantasyon):
- ~~Regime-conditioning trade-off~~ — küçük asimetrik etki, paper scope'una sığmaz
- ~~ML metric ↛ trading metric paradox~~ — ilginç ama 4-6 sayfaya sığmaz, ileride genişletilebilir
- ~~MLP Hold-class fix~~ — Sharpe'a yansımadığı için paper hikayesini güçlendirmiyor
