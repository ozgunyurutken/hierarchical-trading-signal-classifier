"""V5 Phase 3 — Stage 1 Optuna hyperparameter tuning.

Inner walk-forward CV (3 folds) over the full Stage 1 training dataset.
Single hyperparameter set per (asset, model) — applied uniformly to all
outer expanding-window folds in the final retrain.

Search spaces (academic defaults, conservative):
  XGBoost     6 HP   n_estimators, max_depth, learning_rate, subsample,
                     colsample_bytree, min_child_weight
  LightGBM    6 HP   n_estimators, num_leaves, learning_rate,
                     feature_fraction, bagging_fraction, min_data_in_leaf
  RandomForest 4 HP  n_estimators, max_depth, min_samples_leaf, max_features
  MLP          5 HP  hidden_layer_sizes, learning_rate_init, alpha,
                     batch_size, activation (fixed relu)

Objective: F1 macro mean across 3 inner WF-CV folds.
Pruner: MedianPruner (warmup 5 trials, interval 1 step).
"""
...
# Quiet down warnings during heavy tuning loops
...
@dataclass
class InnerCVConfig:
    """Inner walk-forward CV used as Optuna objective.

    Produces exactly `n_folds` evenly-spaced expanding-window val windows
    across the dataset. Tree-/MLP-friendly val_size and gap match the
    outer CV used for the final retrain (gap=10).

    Steps are adaptive: given dataset length n, the first val starts at
    `train_min_size`, the last at `n - gap - val_size`, and the remaining
    folds are uniformly distributed in between.
    """
...
def inner_walk_forward_splits(n: int, cfg: InnerCVConfig):
    """Yield (train_idx, val_idx) for `cfg.n_folds` evenly-spaced WF folds."""
    first_t = cfg.train_min_size
    last_t = n - cfg.gap - cfg.val_size
    if last_t < first_t:
        raise ValueError(
            f"Dataset too small for InnerCVConfig: n={n}, "
            f"train_min={cfg.train_min_size}, val={cfg.val_size}, gap={cfg.gap}"
        )
    if cfg.n_folds == 1:
        ts = [last_t]
    else:
        # Evenly spaced including first_t and last_t
...
def _eval_fold_f1m(model, X_tr, y_tr, X_val, y_val, model_name: str,
                   balanced: bool) -> float:
    """Fit model on (X_tr, y_tr), predict on X_val, return F1 macro."""
...
def _objective(trial: optuna.Trial, X: pd.DataFrame, y: pd.Series,
    ...
    """Mean F1 macro across 3 inner WF folds."""
    ...
    for step_i, (tr_idx, val_idx) in enumerate(
        ...
        # Median pruner sees intermediate value after each fold
...
def run_study(asset: str, model_name: str, X: pd.DataFrame, y: pd.Series,
    ...
    """Run Optuna study, return (study, best_params dict).

    `best_params` is the model factory `**hp` form (sklearn-compatible).
    `storage` — SQLite/PostgreSQL URL for optuna-dashboard live tracking
    (e.g. "sqlite:///optuna_v5.db"). None -> in-memory.
    """
    ...
    study = optuna.create_study(
        study_name=f"v5_stage1_{asset}_{model_name}",
        storage=storage,
    ...
    # Translate MLP categorical idx back into hidden_layer_sizes tuple
    if model_name == "mlp":
        layer_map = {
            "64":       (64,),
            "64_32":    (64, 32),
...
def study_to_trials_df(study: optuna.Study, asset: str, model_name: str) -> pd.DataFrame:
    """Flatten study trials into a DataFrame with trial-level info."""
    rows = []
