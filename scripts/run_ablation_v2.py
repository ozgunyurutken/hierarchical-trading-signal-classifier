...
Outputs:
  data/labels/btc_ablation_v2_summary.csv
  data/labels/btc_ablation_v2_test_signals.csv
  reports/ablation_v2_comparison.png   (4-panel: Sharpe, Return, MaxDD, Win%)
  reports/ablation_v2_equity.png       (4 equity curves overlaid + B&H)
"""
...
def make_plots(results: list[dict], close_test: pd.Series, bh_summary: dict) -> None:
    ...
    fig.suptitle("Ablation v2 — 4 Mimari Konfigürasyonu (XGBoost, BTC test seti 462 gün)",
                 fontsize=12, fontweight="bold", y=0.995)
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    out1 = PROJECT_ROOT / "reports" / "ablation_v2_comparison.png"
    ...
    ax.set_yscale("log")
    ax.grid(alpha=0.3)
    ax.set_title("Ablation v2 — Equity Curves (test seti)", fontsize=12, fontweight="bold")
    ax.set_ylabel("Equity (normalized, log scale)")
    ax.legend(loc="upper left", fontsize=9)
    fig2.tight_layout()
    out2 = PROJECT_ROOT / "reports" / "ablation_v2_equity.png"
...
def main() -> None:
    ...
    save_csv(summary, PROJECT_ROOT / "data" / "labels" / "btc_ablation_v2_summary.csv")
    ...
    save_csv(test_signals, PROJECT_ROOT / "data" / "labels" / "btc_ablation_v2_test_signals.csv")
